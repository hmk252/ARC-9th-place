...
Functions3 initFuncs3(const vector<point>& sizes)
{
    ...
    for (int c = 1; c < 10; c++)
        funcs.add("colShape " + to_string(c), 10, [c](Image_ img) { return colShape(img, c); }, 0);
    ...
    for (int dy = -2; dy <= 2; dy++) {
        for (int dx = -2; dx <= 2; dx++) {
            funcs.add("Move " + to_string(dx) + " " + to_string(dy), 10, [dx, dy](Image_ img) { return Move(img, Pos(dx, dy)); }, 0);
        }
    }
