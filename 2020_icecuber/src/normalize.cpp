    ...
    for (int a = 0; a < 2; a++) {
        for (int b = 0; b < 2; b++) {
            for (int c = 0; c < 2; c++) {
                point dir;
                if (c)
                    dir = point{ a * 2 - 1, b * 2 - 1 } * 2;
                else
                    dir = point{ (a * 2 - 1) * b, (a * 2 - 1) * !b } * 3;
                int ma = -50;
...
void normalizeCols(vector<Sample>& sample)
{
    ...
    for (Sample& s : sample) {
        ...
        int meaningCols = checkAll(s.train, [&](pair<Image, Image> p) { return core::countCols(p.first) == eq; });
...
struct OrientationPicker {
    ...
    int getRigid(Image_ in) const
    {
        if (feature_dim == 0)
            return 0;
        else if (feature_dim == 1)
            return in.h > in.w ? 6 : 0;
