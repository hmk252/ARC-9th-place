...
void run(int only_sid = -1, int arg = -1)
{
    ...
    for (int si = 0; si < sample.size(); si++) {
...

        {
            ...
            for (const Candidate& cand : cands) {
                ...
                if (seen.insert(h).second) {
                    ...
                    if (filtered.size() == CANDIATES_TO_KEEP + skips * 3) break;
                }
            }
        ...
        // Reconstruct answers
        vector<vector<Image>> rec_answers;
        vector<double> answer_scores;
        for (Candidate& cand : answers) {
            vector<Image> rec_answer;
            bool skip = false;
            for (int i = 0; i < s.train.size(); i++) {
                auto& [train_input_image, out] = s.train[i];
                if (cand.imgs[i].w == 0 || cand.imgs[i].h == 0 || cand.imgs[i].w > 30 || cand.imgs[i].h > 30) {
                    skip = true;
                    break;
                }
                rec_answer.push_back(sim.rec(train_input_image, cand.imgs[i]));
            }
            if (skip) continue;
            rec_answer.push_back(sim.rec(s.test_in, cand.imgs.back()));
            double score = cand.score;
            if (add_flips) {
                score /= 2 - 1e-5;
            }
            answer_scores.push_back(score);
            rec_answers.push_back(rec_answer);
        }
        ...
        // }
        {
            string fn = "tmp/output/answer_" + to_string(only_sid) + "_" + to_string(arg) + ".csv";
            // Writer writer(fn);
            // writer(s, rec_answers);
            writeAnswersWithScores_nueva(s, fn, rec_answers, answer_scores);
        }
    }
