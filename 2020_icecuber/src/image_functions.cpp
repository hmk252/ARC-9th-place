...
#define DO(nr, nc)                                  \
    {                                               \
        if (!ret(nr, nc)) {                         \
            ret(nr, nc) = 1;                        \
            if (!a(nr, nc)) q.emplace_back(nr, nc); \
        }                                           \
    }
        if (r > 0) {
            if (c > 0) DO(r - 1, c - 1);
...
Image count(Image_ img, int id, int outType)
{
    ...
    if (id == 0)
        ...
        num = core::countCols(img);
    else if (id == 2)
        num = core::countComponents(img);
    else if (id == 3)
        num = img.w;
    else if (id == 4)
        num = img.h;
...
int maxCriterion(Image_ img, int id)
{
    ...
    case 3:
        return -img.w * img.h;
    case 4:
        return core::countCols(img);
    case 5:
        return -img.p.y;
    case 6:
        return img.p.y;
    case 7:
        return core::countComponents(img);
    case 8: {
    ...
    case 10:
        return core::count(interior(img));
    case 11:
        return -core::count(interior(img));
    case 12:
        return -img.p.x;
...
{ // pad = 0
    ...
    for (int i = 0; i < ret.h; i++) {
        ...
        for (int j = 0; j < ret.w; j++) {
            ...
            if (aj < a.w)
                x = aj;
            else if (aj >= W && aj < W + a.w)
                x = W + a.w - 1 - aj;
            if (ai < a.h)
                y = ai;
            else if (ai >= H && ai < H + a.h)
                y = H + a.h - 1 - ai;
            if (x != -1 && y != -1)
                ret(i, j) = a(y, x);
            if (++aj == W2) aj = 0;
