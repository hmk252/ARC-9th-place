...
{ // maxn = -1
    const string base_path[2] = { "tmp/input_task/", "tmp/input_task/" };
    int base_pathi = 0;
    while (!experimental::filesystem::exists(base_path[base_pathi] + path)) {
        // #include <iostream>
        //         std::cout << base_path[base_pathi] + path << std::endl;
        //         std::cout << "base_path[base_pathi] + path" << std::endl;
        base_pathi++;
        assert(base_pathi < 2);
    }
...
void writeAnswersWithScores_nueva(const Sample& s, string fn, vector<vector<Image>> imgs, vector<double> scores)
{
    FILE* fp = fopen(fn.c_str(), "w");
    assert(fp);
    fprintf(fp, "%s_%d\n", s.id.c_str(), s.id_ind);

    vector<Image> dummyVector = { dummyImg }; // keep 1 for test
    int num_train = s.train.size();
    for (int i = 0; i < num_train; i++) {
        dummyVector.push_back(dummyImg);
    }
    ...
    for (int i = 0; i < imgs.size(); i++) {
        vector<Image> train_test_images = imgs[i];
        double score = scores[i];
        for (int j = 0; j < train_test_images.size(); j++) {
            if (j > 0) fprintf(fp, "-");
            Image_ img = train_test_images[j];
            assert(img.p == point({ 0, 0 }));
            assert(img.w >= 1 && img.w <= 30 && img.h >= 1 && img.h <= 30);
            fprintf(fp, "|");
            for (int i = 0; i < img.h; i++) {
