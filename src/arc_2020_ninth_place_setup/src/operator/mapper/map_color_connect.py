...
from src.operator.mapper.map_connect import neighbors
...
class MapColorConnect:
    ...
    @classmethod
    def array(cls, x_arr: np.array, allow_diagonal: bool = True, background: int = 0) -> List[np.array]:
        """
        ...
        res_list = []
        r, c = x_arr.shape
        con_map = np.zeros((r, c), dtype=int)
        ind = 0
        for i in range(r):
            for j in range(c):
