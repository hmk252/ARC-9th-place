...
def monotone_color(p: Problem) -> int:
...
def new_color(p: Problem) -> List[int]:
    ...
    new_color_count = np.zeros(10, dtype=int)
    new_color_count_diff = np.zeros(10, dtype=int)
...
def deleted_color(p: Problem) -> List[int]:
    case_x: Case
    case_y: Case
    deleted_color_flag = np.ones(10, dtype=int)
    for case_x, case_y in zip(p.train_x_list, p.train_y_list):
        # color count
...
def only_color(p: Problem) -> bool:
    ...
    for case_x, case_y in zip(p.train_x_list, p.train_y_list):
        x_values_bool = (case_x.repr_values() == case_x.background_color).astype(int)
        y_values_bool = (case_y.repr_values() == case_y.background_color).astype(int)
