...
def find_replace_rule_33_one(x_arr, y_arr, background):
    res_arr = np.int64(-1) * np.ones((10, 3, 3), dtype=np.int64)
        ...
        for j in range(n_col):
            ...
            if i > 0 and j > 0:
                if x_arr[i - 1, j - 1] == background:
                    new_c = y_arr[i - 1, j - 1]
                    if res_arr[c, 0, 0] == int(-1):
                        res_arr[c, 0, 0] = new_c
                    if res_arr[c, 0, 0] != new_c:
                        res_arr[c, 0, 0] = -2
            # left
            if j > 0:
                if x_arr[i, j - 1] == background:
                    new_c = y_arr[i, j - 1]
                    if res_arr[c, 1, 0] == int(-1):
                        res_arr[c, 1, 0] = new_c
                    if res_arr[c, 1, 0] != new_c:
                        res_arr[c, 1, 0] = -2
            # left down
            if i < n_row - 1 and j > 0:
                if x_arr[i + 1, j - 1] == background:
                    new_c = y_arr[i + 1, j - 1]
                    if res_arr[c, 2, 0] == int(-1):
                        res_arr[c, 2, 0] = new_c
                    if res_arr[c, 2, 0] != new_c:
                        res_arr[c, 2, 0] = -2
            # up
            if i > 0:
                if x_arr[i - 1, j] == background:
                    new_c = y_arr[i - 1, j]
                    if res_arr[c, 0, 1] == int(-1):
                        res_arr[c, 0, 1] = new_c
                    if res_arr[c, 0, 1] != new_c:
                        res_arr[c, 0, 1] = -2
            # center
            new_c = y_arr[i, j]
            if res_arr[c, 1, 1] == int(-1):
                res_arr[c, 1, 1] = new_c
            if res_arr[c, 1, 1] != new_c:
                res_arr[c, 1, 1] = -2
            # down
            if i < n_row - 1:
                if x_arr[i + 1, j] == background:
                    ...
                    if res_arr[c, 2, 1] == -1:
                        res_arr[c, 2, 1] = new_c
                    if res_arr[c, 2, 1] != new_c:
                        res_arr[c, 2, 1] = -2
            # right up
            if i > 0 and j < n_col - 1:
                if x_arr[i - 1, j + 1] == background:
                    new_c = y_arr[i - 1, j + 1]
                    if res_arr[c, 0, 2] == int(-1):
                        res_arr[c, 0, 2] = new_c
                    if res_arr[c, 0, 2] != new_c:
                        res_arr[c, 0, 2] = -2
            # right
            if j < n_col - 1:
                if x_arr[i, j + 1] == background:
                    new_c = y_arr[i, j + 1]
                    if res_arr[c, 1, 2] == int(-1):
                        res_arr[c, 1, 2] = new_c
                    if res_arr[c, 1, 2] != new_c:
                        res_arr[c, 1, 2] = -2
            # right down
            if i < n_row - 1 and j < n_col - 1:
                if x_arr[i + 1, j + 1] == background:
                    new_c = y_arr[i + 1, j + 1]
                    if res_arr[c, 2, 2] == int(-1):
                        res_arr[c, 2, 2] = new_c
                    if res_arr[c, 2, 2] != new_c:
                        res_arr[c, 2, 2] = -2

    for c in range(10):
        for i in range(3):
            for j in range(3):
                if res_arr[c, i, j] < int(0):
                    res_arr[c, i, j] = background

    return res_arr


def find_replace_rule_33(x_arr_list: List[np.array], y_arr_list: List[np.array], background: int = 0) -> np.array:

    res_arr = background * np.ones((10, 3, 3), dtype=int)
    n = len(x_arr_list)
...
def fit_replace_rule_33_one(x_arr, rule, background):
    ...
    for i in range(n_row):
        for j in range(n_col):
            ...
            # left up
            if i > 0 and j > 0:
                if x_arr[i - 1, j - 1] == background != rule[c, 0, 0]:
                    y_arr[i - 1, j - 1] = rule[c, 0, 0]
            # left
            if j > 0:
                if x_arr[i, j - 1] == background != rule[c, 1, 0]:
                    y_arr[i, j - 1] = rule[c, 1, 0]
            # left down
            if i < n_row - 1 and j > 0:
                if x_arr[i + 1, j - 1] == background != rule[c, 2, 0]:
                    y_arr[i + 1, j - 1] = rule[c, 2, 0]
            # up
            if i > 0:
                if x_arr[i - 1, j] == background != rule[c, 0, 1]:
                    y_arr[i - 1, j] = rule[c, 0, 1]
            # center
            if rule[c, 1, 1] != background:
                y_arr[i, j] = rule[c, 1, 1]
            # down
            if i < n_row - 1:
                if x_arr[i + 1, j] == background != rule[c, 2, 1]:
                    y_arr[i + 1, j] = rule[c, 2, 1]
            # right up
            if i > 0 and j < n_col - 1:
                if x_arr[i - 1, j + 1] == background != rule[c, 0, 2]:
                    y_arr[i - 1, j + 1] = rule[c, 0, 2]
            # right
            if j < n_col - 1:
                if x_arr[i, j + 1] == background != rule[c, 1, 2]:
                    y_arr[i, j + 1] = rule[c, 1, 2]
            # right down
            if i < n_row - 1 and j < n_col - 1:
                if x_arr[i + 1, j + 1] == background != rule[c, 2, 2]:
...
def find_replace_rule_33_one_all(x_arr, y_arr, background):
    res_arr = int(-1) * np.ones((3, 3), dtype=np.int64)
    assert x_arr.shape == y_arr.shape
...
def find_replace_rule_33_all(x_arr_list: List[np.array], y_arr_list: List[np.array], background: int = 0) -> np.array:
    ...
    res_arr = background * np.ones((3, 3), dtype=int)
    n = len(x_arr_list)
...
if __name__ == "__main__":
    ...
    x_list = [np.array([[0, 0, 0], [0, 1, 0], [0, 0, 0]], dtype=int), np.array([[1, 0, 0], [0, 0, 0], [0, 0, 0]], dtype=int)]
    y_list = [np.array([[5, 0, 5], [0, 4, 0], [5, 0, 5]], dtype=int), np.array([[4, 0, 0], [0, 5, 0], [0, 0, 0]], dtype=int)]
    rule_easy = find_replace_rule_33(x_list, y_list)
    print(rule_easy)
    print(fit_replace_rule_33_one(x_list[0], rule_easy, 0))
    x_list = [np.array([[0, 0, 0], [0, 1, 0], [0, 2, 0]], dtype=int), np.array([[1, 0, 0], [0, 1, 0], [0, 2, 0]], dtype=int)]
    y_list = [np.array([[5, 5, 5], [5, 1, 5], [5, 2, 5]], dtype=int), np.array([[1, 5, 5], [5, 1, 5], [5, 2, 5]], dtype=int)]
