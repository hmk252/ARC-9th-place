...
def is_rot_symmetry_point(x_arr: np.array, background: int) -> np.array:
    ...
    for i0 in range(x_arr.shape[0]):
        for j0 in range(x_arr.shape[1]):
            ...
            score_arr = (
                (x_arr_1_exp != background).astype(np.int64)
                + (x_arr_2_exp != background).astype(np.int64)
                + (x_arr_3_exp != background).astype(np.int64)
                + (x_arr_4_exp != background).astype(np.int64)
            )
...
@numba.jit("i8[:, :](i8[:, :], i8)", nopython=True)
def is_rot_symmetry_valley(x_arr: np.array, background: int) -> np.array:
...
def fill_rot_symmetry_point(x_arr: np.array, background: int, i0: int, j0: int) -> np.array:
...
def fill_rot_symmetry_valley(x_arr: np.array, background: int, i0: int, j0: int) -> np.array:
...
class AutoFillRotSymmetry:
    ...
    @classmethod
    def array(cls, x_arr: np.array, background: int, full: bool) -> np.array:
