...
def is_line_symmetry_row_strict(x_arr: np.array, background: int) -> np.array:
...
def is_line_symmetry_col_strict(x_arr: np.array, background: int) -> np.array:
    return is_line_symmetry_row_strict(x_arr.transpose(), background)


def fill_symmetry_row(x_arr: np.array, background: int, i0: int, center: bool) -> np.array:
...
def fill_symmetry_col(x_arr: np.array, background: int, j0: int, center: bool) -> np.array:
...
class AutoFillLineSymmetryAdd:
    ...
    @classmethod
    def auto_fill_symmetry_row(cls, x_arr: np.array, background: int = 0) -> np.array:
    ...
    @classmethod
    def auto_fill_symmetry_col(cls, x_arr: np.array, background: int = 0) -> np.array:
        assert x_arr.shape[0] > 0 and x_arr.shape[1] > 0
        max_c = -1
        res_max = 0
        for color in range(10):
    ...
    @classmethod
    def auto_fill_symmetry_row_col(cls, x_arr: np.array, background: int = 0) -> np.array:
        assert x_arr.shape[0] > 0 and x_arr.shape[1] > 0
        max_c = -1
        res_max = 0
        for color in range(10):
