...
class Shadow:
    ...
    def case(cls, c: Case, shadow_type: str, color: int = -1) -> Case:
        ...
        if shadow_type == "bool":
            new_case.shadow = (repr_values != c.background_color).astype(bool)
        elif shadow_type == "same":
            new_case.shadow = repr_values.copy()
        elif shadow_type == "max":
            new_case.shadow = (repr_values == new_case.max_color()).astype(bool)
        elif shadow_type == "min":
            new_case.shadow = (repr_values == new_case.min_color()).astype(bool)
        elif shadow_type == "mesh":
            new_case.shadow = keep_mesh_array(repr_values)
        elif shadow_type == "ones":
            new_case.shadow = np.ones(repr_values.shape, dtype=bool)
        elif shadow_type == "problem_max":
            new_case.shadow = (repr_values == color).astype(bool)
        return new_case
