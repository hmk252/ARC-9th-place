            ...
            if pattern is not None:
                ...
                if has_complete:
                    for transform in get_all_transforms():
                        ...
                        pred = np.tile(
                            transformed_pattern,
                            (x.shape[0] // transformed_pattern.shape[0] + 2, x.shape[1] // transformed_pattern.shape[1] + 2),
                        )
    ...
    for c in np.unique(test_input):
        for shape in has_shapes:
            ...
            for transform in has_transforms:
                ...
                pred = np.tile(
                    transformed_pattern,
                    (test_input.shape[0] // transformed_pattern.shape[0] + 2, test_input.shape[1] // transformed_pattern.shape[1] + 2),
                )
...
def third_place_predict(task_id, task):
    outputs = []
    task = deepcopy(task)
    pair_id = 0
    ...
    if len(predictions) > 0:
        python_array_predictions = []
        # Convert numpy to raw array
        for pred in predictions:
            python_array_predictions.append(pred.tolist())
        predictions = python_array_predictions
    return predictions
...
def final_sub2_predict(task_id, task):
    pair_id = 0
